...
import com.webnotics.swipee.activity.Seeker.AddCollegeActivity;

import java.util.ArrayList;

public class CollegeAdapter extends BaseAdapter implements Filterable {

    AddCollegeActivity mContext;
    ArrayList<JsonObject> mArrayListSkills;
    CustomFilter filter;
    ArrayList<JsonObject> filterList;


    public CollegeAdapter(AddCollegeActivity mContext, ArrayList<JsonObject> mArrayListSkills) {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                for (int i = 0; i < filterList.size(); i++) {
                    if (filterList.get(i).get("university_college_name").getAsString().toUpperCase().contains(constraint)) {
                        JsonObject p = new JsonObject();
                        p.add("university_college_id", filterList.get(i).get("university_college_id"));
                        p.add("university_college_name", filterList.get(i).get("university_college_name"));
                        filters.add(p);
                    }
                }
