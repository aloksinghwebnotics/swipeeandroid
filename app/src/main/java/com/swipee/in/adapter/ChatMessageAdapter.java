...
import com.swipee.in.activity.ChatActivity;
...
public class ChatMessageAdapter extends RecyclerView.Adapter<ChatMessageAdapter.MyViewHolder> {
    ...
    public ChatMessageAdapter(ChatActivity context, Context mContext, ArrayList<ChatModel.Data> modelArrayList) {
    ...
    public void callDownload(int position) {
        String url = modelArrayList.get(position).getMsg_filename();
        String name = modelArrayList.get(position).getMsg_filename_org();
        String id = modelArrayList.get(position).getAppointment_id().replaceAll(" ", "");
        //  AppController.callResume(mContext,url);
        if (context.checkPermission(position)) {
            File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsoluteFile(), id + "_" + name);
            if (file.exists()) {
                ...
                } else if (file.getName().contains(".jpg") || file.getName().contains(".jpeg") || file.getName().contains(".png") || file.getName().contains(".bmp")) {
                    // JPG file
                ...
                AppController.downloadFiles(mContext, url, id + "_" + name);
