...
public class FeaturedJobsActivity extends AppCompatActivity {
    Rest rest;
    Context mContext;
    ImageView iv_back;
    TextView tv_noappliedjob, tv_title;
    LinearLayout ll_nodata;
    RecyclerView rv_appliedJobs;
    @SuppressLint("StaticFieldLeak")
    public static FeaturedJobsActivity instance;
    ...
    private void getAppliedJobs() {
        SwipeeApiClient.swipeeServiceInstance().getFeaturedJobs(Config.GetUserToken()).enqueue(new Callback<JsonObject>() {
            @Override
                    ...
                    if (response.body().get("code").getAsInt() == 203) {
                        ...
                        if (job_data.size() > 0) {
                            ...
                            IntStream.range(0, job_data.size()).mapToObj(i -> job_data.get(i).getAsJsonObject()).forEach(object -> {
                                ...
                                String user_job_status = object.has("job_status") ? object.get("job_status").isJsonNull() ? "" : object.get("job_status").getAsString() : "";
